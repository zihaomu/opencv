...
#include "int8layers/cpu_kernels/conv_block.simd_declarations.hpp" // defines CV_CPU_DISPATCH_MODES_ALL=AVX2,...,BASELINE based on CMakeLists.txt content
            ...
            bool per_channel)
{
    ...
    else if (conv->conv_type == QCONV_TYPE_GENERIC)
    {
        ...
        // TODEL
//        char* srcWeights0 = (char *)weightsMat.data;
//
//        std::cout<<"before pack"<<std::endl;
//        for (int n = 0; n < ngroups; n++)
//        {
//            for (int k = 0; k < Kg; k++)
//            {
//                char* srcWeights1 = srcWeights0 + (n * ngroups + k) * wstep;
//                for (int c = 0; c < Cg; c++)
//                {
//                    char* srcWeights2 = srcWeights1 + c * wstep;
//                    for (int kk = 0; kk < DkHkWk; kk++)
//                    {
//                        std::cout<<","<<(int)srcWeights2[kk];
//                    }
//                    std::cout<<"_";
//                }
//                std::cout<<std::endl;
...
//        // print the packed data of weight
//        auto weightsBufPtrBack = weightsBufPtr;
//        for (int n = 0; n < ngroups; n++)
//        {
//            for (int s = 0; s < numStripsMR; s++)
//            {
//                char* weightsBufPtrBack0 = weightsBufPtrBack + (n * Kg_aligned + CONV_MR * s) * DkHkWkCg_aligned;
//
//                for (int i = 0; i < DkHkWk * Cg_strips; i++)
//                {
//                    char* weightsBufPtrBack1 = weightsBufPtrBack0 + i * CONV_MR * CONV_PACKN;
//                    for (int j = 0; j < CONV_MR * CONV_PACKN; j++)
//                    {
//                        std::cout<<","<<(int)weightsBufPtrBack1[j];
//                    }
//                    std::cout<<std::endl;
//                }
//            }
...
static inline void store_block_layout_24(const int8_t* src, int8_t* dst, const int src_planesize)
{
    ...
    v0 = v_load_halves(src + 16, src + 16 + src_planesize);
    v1 = v_load_halves(src + 16 + 2 * src_planesize, src + 16 + 3 * src_planesize);

    v_zip(v0, v1, v2, v3);
    v_store_interleave(dst + 64, v2, v3);

#if CONV_PACKN == 8
    v0 = v_load(src + 4 * src_planesize);
    v1 = v_load(src + 5 * src_planesize);
    v2 = v_load(src + 6 * src_planesize);
    v3 = v_load(src + 7 * src_planesize);

    v_store_interleave(dst + 96, v0, v1, v2, v3);

    v0 = v_load_halves(src + 4 * src_planesize + 16, src + 5 * src_planesize + 16);
    v1 = v_load_halves(src + 16 + 6 * src_planesize, src + 16 + 7 * src_planesize);

    v_zip(v0, v1, v2, v3);
    v_store_interleave(dst + 160, v2, v3);
#endif
#else
...
#if CONV_PACKN == 8
    v0 = v_load_halves(src + 4 * src_planesize, src + 5 * src_planesize);
    v1 = v_load_halves(src + 6 * src_planesize, src + 7 * src_planesize);

    v_zip(v0, v1, v_p0, v_p1);
    v_store_interleave(dst + 32, v_p0, v_p1);
#endif
...
{
    std::vector<int8_t> buffer(4 * CONV_NR * sizeof (int8_t ) + VEC_ALIGN);
    int8_t* bufferPtr = alignPtr(buffer.data(), VEC_ALIGN);

    std::vector<int8_t> zbuffer(CONV_NR * sizeof (int8_t ) + VEC_ALIGN, input_zp);
    ...
    for (int stripe = 0; zyx0 < zyx_limit; stripe++, zyx0 += CONV_NR)
    {
        ...
        else
        {
            ...
            for (int k = 0; k < ksize; k++)
            {
                ...
                for (; i < CONV_NR;)
                {
                    int8_t* inpbuf_ki = inpbuf + k * CONV_NR * Cg_aligned + i * 4;
                        ...
                        if (i + 8 <= CONV_NR && x0 + 8 <= W0 && xi + stride_w * 8 <= Wi)
                        {
                            ...
                            else
                            {
                                ...
                                if (c < Cg)
                                {
                                    ...
                                    #if CONV_PACKN == 8
                                    const int8_t* inptr4 = c+1 < Cg ? inptr_ki + inp_planesize*4 : zbufPtr;
                                    const int8_t* inptr5 = c+1 < Cg ? inptr_ki + inp_planesize*5 : zbufPtr;
                                    const int8_t* inptr6 = c+2 < Cg ? inptr_ki + inp_planesize*6 : zbufPtr;
                                    const int8_t* inptr7 = c+3 < Cg ? inptr_ki + inp_planesize*7 : zbufPtr;
...
//    TODO! support q depth_wise.
    ...
    for (int task_id = r0.start; task_id < r0.end; task_id++)
    {
        ...
        else
        {
            ...
            for (; ngs0 < ngs1; ngs0 += ds)
            {
                ...
                auto a = ngroups * Cg * inp_planesize;
    ...
    // TODEL
    // N * ngroups * stripes_per_sample0 * stripesize;
//    stripesize = alignSize(CONV_NR * ksize * Cg_aligned, VEC_ALIGN);

//    char* inpbuf_all_tmp = inpbuf_pack;
//    std::cout<<"print pack data."<<std::endl;
//    for (int n = 0; n < ngroups; n++)
//    {
//        for (int s = 0; s < stripes_per_sample0; s++)
//        {
//            char * inpbuf_all_tmp_0 = inpbuf_all_tmp + n * stripes_per_sample0 * stripesize + s * stripesize;
//            for (int kc = 0; kc < DkHkWkCg_aligned/4; kc++)
//            {
//                char * inpbuf_all_tmp_1 = inpbuf_all_tmp_0 + kc * CONV_NR * 4;
//
//                for (int nr = 0; nr < CONV_NR; nr++)
//                {
//                    for (int i = 0; i < 4; i++)
//                    {
//                        std::cout<<","<<(int)inpbuf_all_tmp_1[nr * 4 + i];
//                    }
//                    std::cout<<",_";
//                }
//                std::cout<<std::endl;
        ...
        for (int subtask = ngs0; subtask < ngs1; )
        {
            ...
            for (; zyx0 < zyx_limit; zyx0 = zyx_block_limit)
            {
                ...
                for (int k0_block = k0; k0_block < k1; k0_block += K_BLOCK_SIZE)
                {
                    ...
                    // TODEL
//                    char* inpbuf_all_tmp = inpbuf_pack;
//                    std::cout<<"inner print pack data."<<std::endl;
//                    for (int s = 0; s < stripes_per_sample0; s++)
//                    {
//                        char * inpbuf_all_tmp_0 = inpbuf_all_tmp + s * stripesize;
//                        for (int kc = 0; kc < DkHkWkCg_aligned/4; kc++)
//                        {
//                            char * inpbuf_all_tmp_1 = inpbuf_all_tmp_0 + kc * CONV_NR * 4;
//
...
//                        std::cout<<"k = "<<k<<", biasval = "<<biasval<<", mul = "<<mult<<std::endl;
                        int j = 0;
#if CV_SIMD128
                        v_float32x4 vbias = vx_setall_f32(biasval);
                        v_float32x4 vmult = v_setall_f32(mult);

                        // TODO! support per_channel output_zp
...
static void convBlockNoSIMD_INT8(int np, const int8_t* a, const int8_t* b, int* c, int ldc, bool init_c, const int outLen,
                            const int convMR, const int convNR)
{
               ...
               const int convMR, const int convNR)
{
    ...
    if (outLen > 12)
    {
        ...
        for (int p = 0; p < np; p += 4, a += 4 * convMR, b += 4 * convNR)
        {
            ...
            c00 = v_add(v_dotprod_expand(a0_8, b0), c00);
            c01 = v_add(v_dotprod_expand(a0_8, b1), c01);
            c02 = v_add(v_dotprod_expand(a0_8, b2), c02);
            c10 = v_add(v_dotprod_expand(a1_8, b0), c10);
            c11 = v_add(v_dotprod_expand(a1_8, b1), c11);
            c12 = v_add(v_dotprod_expand(a1_8, b2), c12);
            c20 = v_add(v_dotprod_expand(a2_8, b0), c20);
            c21 = v_add(v_dotprod_expand(a2_8, b1), c21);
            c22 = v_add(v_dotprod_expand(a2_8, b2), c22);
            c30 = v_add(v_dotprod_expand(a3_8, b0), c30);
            c31 = v_add(v_dotprod_expand(a3_8, b1), c31);
            c32 = v_add(v_dotprod_expand(a3_8, b2), c32);

            b0 = v_load(b + 48); b1 = v_load(b + 64); b2 = v_load(b + 80);

            c03 = v_add(v_dotprod_expand(a0_8, b0), c03);
            c04 = v_add(v_dotprod_expand(a0_8, b1), c04);
            c05 = v_add(v_dotprod_expand(a0_8, b2), c05);
            c13 = v_add(v_dotprod_expand(a1_8, b0), c13);
            c14 = v_add(v_dotprod_expand(a1_8, b1), c14);
            c15 = v_add(v_dotprod_expand(a1_8, b2), c15);
            c23 = v_add(v_dotprod_expand(a2_8, b0), c23);
            c24 = v_add(v_dotprod_expand(a2_8, b1), c24);
            c25 = v_add(v_dotprod_expand(a2_8, b2), c25);
            c33 = v_add(v_dotprod_expand(a3_8, b0), c33);
            c34 = v_add(v_dotprod_expand(a3_8, b1), c34);
            c35 = v_add(v_dotprod_expand(a3_8, b2), c35);
        }
...
#else
    convBlockNoSIMD_INT8(np, a, b, c, ldc, init_c, outLen, convMR, convNR);
#endif
}
