...
//        {
//            for(int k0 = 0; k0 < Kg_aligned; k0 += VEC_LEN)
//            {
//                int dk = Kg - k0 < VEC_LEN ? Kg - k0 : VEC_LEN;
//                for(int c = 0; c < Cg; c++)
//                {
//                    for(int yx = 0; yx < Hk*Wk; yx++, packed_wptr += VEC_LEN)
//                    {
...
//            for(int hwd = 0; hwd < Hk*Wk; hwd++) {
//                for(int c = 0; c < Cg; c++, packed_wptr += VEC_LEN)
//                {
//                    const float* wptr = srcWeight + wstep * k_idx + c*Hk*Wk + hwd;
//                    int k = 0;
//                    for(; k < dk; k++, wptr += wstep)
...
bool OpConv::forward(std::vector<Tensor>& ins, std::vector<Tensor>& outs)
{
    ...
    {
        if (shaderType == ConvShaderType::kConvShaderTypeGeneric)
        {
            SubOpConvRepackInput repackOp(ngroups, batch, Hi, Wi, H0, W0,  C, Hk, Wk, stride_h, stride_w, dilation_h,
                                          dilation_w, pad_left, pad_top);
            operation = makePtr<SubOpConvRepackInput>(repackOp);
//        std::cout<<"print original input = "<<std::endl;
//        ins[0].printTensor(4);

        }

        bufferNum_ = 4;
        OpBase::initVulkanThing(bufferNum_);
        if (shaderType == kConvShaderTest)
            config.local_size_x = 1; // TODO! should be set as the subgroupSize.
        else
        {
            config.local_size_x = GPU_WORK_GROUP_SIZE; // TODO! should be set as the subgroupSize.
        }
        ...
        {
            CV_Assert(ngroups == 1); // TODO: support group > 1
            createShaderModule(conv_spv, sizeof(conv_spv));
            createPipeline(sizeof(ShaderParam));
        }
        else if (shaderType == kConvShaderTest)
        {
            size_t dataLen = sizeof(moo_test_spv);
            unsigned int* conv_spv_modified = (unsigned int*)malloc(dataLen);
            setXYZ(conv_spv_modified, moo_test_spv, dataLen, config.local_size_x, config.local_size_y, config.local_size_z, &dataLen);
            createShaderModule(conv_spv_modified, dataLen); // TODO: move this step out of the forward stage.
            createPipeline(sizeof(ShaderParam));
        }
        else if (shaderType == kConvShaderTypeGeneric)
        {
            // Step 2: compute conv gemm 4x4.
            size_t dataLen = sizeof(conv_4x4_spv);
            unsigned int* conv_4x4_spv_modified = (unsigned int*)malloc(dataLen);

            setXYZ(conv_4x4_spv_modified, conv_4x4_spv, dataLen, config.local_size_x, config.local_size_y, config.local_size_z, &dataLen);
            createShaderModule(conv_4x4_spv_modified, dataLen); // TODO: move this step out of the forward stage.
            createPipeline(sizeof(ShaderParam4x4));
        }
        else
            CV_Error(CV_StsNotImplemented, "Not Implement with Depth Wise Conv in Vulkan!!!");
    ...
    {
        Layer::t0.start();
        std::vector<int> shapeRepack = {batch * ngroups, hwStripe, CgHkWk, VEC_LEN};
        repackInput = Tensor(0, shapeRepack);
        testTensorPtr = makePtr<Tensor>(repackInput);
        std::vector<Tensor> repackInputs;
        repackInputs.push_back(repackInput);
        operation->forward(ins, repackInputs);
        Layer::t0.stop();
    }
    ...
    if (shaderType == kConvShaderTypeGeneric)
    {
//        std::cout<<"print repack Tensor  = "<<std::endl;
//        testTensorPtr->printTensor(VEC_LEN);
        bindTensor(device_, repackInput, 0, descriptor_set_);
    }
    else if (shaderType == kConvShaderTest)
    {
        Tensor testTensor;
        auto inShape = ins[0].getShape();
        size_t totalNum = total(inShape);
        std::vector<float> vFloat(totalNum, 0);
        for (int i = 0; i < totalNum; i++)
        {
            vFloat[i] = (float )i;
        }

        // 真实的拷贝，其实并不是在代码运行到这里时发生的，而是在随后运行时发生，这一步仅仅是将handle拷贝一下，连拷贝的内存都没有。
        testTensor.reshape((const char*)vFloat.data(), inShape);
        testTensorPtr = makePtr<Tensor>(testTensor);

        // input as other test model.
        bindTensor(device_, testTensor, 0, descriptor_set_);
    }
    else
        bindTensor(device_, ins[0], 0, descriptor_set_);

    bindTensor(device_, *weightTensorPtr, 1, descriptor_set_);

    Layer::t1.start();
//    std::cout<<"print weight Tensor  = "<<std::endl;
//    weightTensorPtr->printTensor(VEC_LEN);

    bindTensor(device_, *biasTensorPtr, 2, descriptor_set_);
    bindTensor(device_, outs[0], 3, descriptor_set_);

    if (shaderType == kConvShaderTest || shaderType == kConvShaderTypeDepthWise)
    {
        ShaderParam param = {Hi, Wi,
                             H0, W0,
                             stride_h, stride_w,
                             pad_top, pad_left,
                             Hk, Wk,
    ...
    {
        ShaderParam4x4 param = {
                H0, W0, CgHkWk, Kg, ngroups, batch
        };
        recordCommandBuffer((void *)&param, sizeof(ShaderParam4x4));
    }
