...
public:
    ...
    template <typename T, typename Functor>
    void where_forward(const Functor& f, const std::vector<Mat>& inputs, std::vector<Mat>& outputs)
    {
        const Mat& a = inputs[0];
        const Mat& b = inputs[1];
    ...
    inline void opDispatch(size_t ninputs, Args&&... args)
    {
        switch (op)
        {
            ...
            case OPERATION::WHERE:
            {
                ...
                where_forward<T>(op_where, std::forward<Args>(args)...);
                break;
            }
            default:
