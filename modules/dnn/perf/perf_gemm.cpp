...
static inline void PrintTo(const GemmParamID& v, std::ostream* os)
{
    ...
    *os << "GFLOPS=" << cv::format("%.3f", p.declared_flops * 1e-9)
        << ", IN0 = {" << p.shapeIn0[0] << ", " << p.shapeIn0[1]
        << "} , IN1 = {" << p.shapeIn1[0] << ", " << p.shapeIn1[1]<<"}";
}
...
//    normAssert(ref, output, "", 1e-5, 1e-4);
...
//    }
    ...
    std::cout
            << "IN0=" << divUp(input0.total() * input0.elemSize(), 1u<<10) << " Kb " << inputShape0
            << "    IN1=" << divUp(input1.total() * input1.elemSize(), 1u<<10) << " Kb " << inputShape1
